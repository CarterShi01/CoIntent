...
from cointent.models import Goal, ProjectModel, RoleRecord, TraceLink
...
def test_proposal_acceptance_creates_immutable_version(tmp_path: Path) -> None:
    ...
    assert repo.get_model("demo", 1)["model"]["goals"] == []
...
def test_incremental_snapshot_creates_mapped_change_finding(tmp_path: Path) -> None:
    ...
    model = ProjectModel(
        ...
        goals=[Goal(id="goal.ship", title="Ship")],
        roles=[RoleRecord(id="role.api", name="API", purpose="Own the API")],

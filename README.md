...
CoIntent is an Agent-native, explicit-invocation system with two processes: understand the current program from code, and design the program humans want next. It is not an always-on task manager. Its MCP surface is used only when the user's current request explicitly asks to use CoIntent or Understand Anything (UA/UV); generic code inspection, explanation, review, design, or modification requests must not be routed into it. After activation, a product manager can begin with a plain-language capability and repeatedly drill into the structure that fulfills it. Concrete source locations remain linked as evidence without turning the primary view into a file or class diagram.
